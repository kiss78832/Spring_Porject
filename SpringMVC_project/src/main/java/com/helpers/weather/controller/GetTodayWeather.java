...
import com.helpers.weather.model.WeatherVO;

public class GetTodayWeather implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent event) {
		...
		event.getServletContext().log("定時器已啟動"); //延遲1小時;每兩小時一次
		timer.schedule(new TodayWeatherTask(event.getServletContext()), 1000*30*60*1, 1000*60*60*2); 
		...
		try {
			...
			for(int i = 1;i<=5 ;i++) {
				...
				event.getServletContext().setAttribute("W"+i, weatherVO);
