...
public class DBGifReader4 extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			String eq_num = req.getParameter("eq_num").trim();
			ResultSet rs = stmt.executeQuery("SELECT EQ_PIC FROM EQUIPMENT WHERE EQ_NUM = '" + eq_num + "' ");

			if (rs.next()) {
				BufferedInputStream in = new BufferedInputStream(rs.getBinaryStream("eq_pic"));
